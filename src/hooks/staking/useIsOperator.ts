...
export function useIsOperator(layer2: string | undefined) {
	...
	const { WTON_CONTRACT, SeigManager_CONTRACT, Layer2Manager_CONTRACT } =
		useCallContract();
		...
		async function fetch() {
			if (CandidateAddOn_CONTRACT && account && layer2) {
				try {
					...
					const OperatorManager_CONTRACT = await getContract(
						operatorAddress,
						OperatorManager,
						library,
						account,
					...
					if (
						...
						const rollupConfigInfo =
							await Layer2Manager_CONTRACT.rollupConfigInfo(rollupConfig);
						setRollupConfigInfo(rollupConfigInfo.status);
						if (rollupConfigInfo.status !== 1) {
							return;
						}
						const RollupConfig = await getContract(
							rollupConfig,
							SystemConfig,
							library,
							account,
						...
						const wtonBalanceOfOM =
							await WTON_CONTRACT.balanceOf(operatorAddress);
						const estimatedDistribution =
							await SeigManager_CONTRACT.estimatedDistribute(
								blockNumber + 1,
								layer2,
								true,
							);
						const addedWton = wtonBalanceOfOM.add(
							estimatedDistribution.layer2Seigs,
						);
