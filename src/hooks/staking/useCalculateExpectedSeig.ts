	...
	useEffect(() => {
		async function fetch() {
			...
			if (
				...
				try {
					const Tot = getContract(
						await SeigManager_CONTRACT.tot(),
						RefactorCoinageSnapshotABI,
						library,
						account,
					);
					const coinage = getContract(
						await SeigManager_CONTRACT.coinages(candidateContract),
						Coinage,
						library,
						account,
					);
					...
					const tonBalanceOfZero = await TON_CONTRACT.balanceOf(
						ethers.constants.AddressZero,
					);
					const tonBalanceOfOne = await TON_CONTRACT.balanceOf(
					...
					const relativeSeigRate =
						await SeigManager_CONTRACT.relativeSeigRate();

					let tos = tonTotalSupply
						.sub(tonBalanceOfWTON)
						.sub(tonBalanceOfZero)
						.sub(tonBalanceOfOne)
						.mul(RAYDIFF)
						.add(totTotalSupply);

					const totFactor = await Tot.factor();
					const totBalanceAndFactor =
						await Tot.getBalanceAndFactor(candidateContract);

					let coinageTotalSupply = await coinage.totalSupply();
					let maxSeig = await calcMaxSeigs(
						lastSeigBlock,
						seigPerBlock,
						blockNumber,
					);
					let stakedSeig1 = maxSeig
						.mul(totTotalSupply)
						.div(BigNumber.from(tos.toString()));
					let unstakedSeig = maxSeig.sub(stakedSeig1);
					let stakedSeig = stakedSeig1.add(
						unstakedSeig.mul(relativeSeigRate).div(RAY),
					);

					let nextTotTotalSupply = totTotalSupply.add(stakedSeig);
					let newTotFactor = await calcNewFactor(
						totTotalSupply,
						nextTotTotalSupply,
						totFactor,
					);
					let newFactorSet = await setFactor(newTotFactor);
					let nextBalanceOfLayerInTot = await applyFactor(
						newFactorSet?.factor,
						newFactorSet?.refactorCount,
						totBalanceAndFactor[0].balance,
						totBalanceAndFactor[0].refactoredCount,
					);

					let commissionRates =
						await SeigManager_CONTRACT.commissionRates(candidateContract);

					let isCommissionRateNegative =
						await SeigManager_CONTRACT.isCommissionRateNegative(
							candidateContract,
						);
					...
					if (commissionRates.toString() != ethers.constants.Zero) {
						if (!isCommissionRateNegative) {
							...
							const userSeig = restSeigs
								.mul(BigNumber.from(userStaked))
								.div(BigNumber.from(stakedAmount));
							seig =
								candidate === account ? userSeig.add(operatorSeigs) : userSeig;
						}
					} else {
						seig =
							stakedAmount === "0"
