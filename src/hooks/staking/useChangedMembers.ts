...
export function useChangedMembers() {
	...
	useEffect(() => {
		async function fetchEvent() {
			...
			const checkSlot = events.filter(
				(event: any) => event.eventName === "ChangedSlotMaximum",
			);
			...
			if (data) {
				...
				for (let i = 0; i < slotNumber; i++) {
					const slotOccupied = changedMembers.filter(
						(changedMember: any) => Number(changedMember.slotIndex) === i,
					);
					membersAddress.push({
