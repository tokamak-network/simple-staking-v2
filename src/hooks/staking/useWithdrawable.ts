...
export function useWithdrawable(layer2: string | undefined) {
	...
	const { DepositManager_CONTRACT, Old_DepositManager_CONTRACT } =
		useCallContract();
		...
		async function fetchWithdrawableData() {
			try {
				...
				const numPendingRequests =
					await DepositManager_CONTRACT?.numPendingRequests(layer2, account);
				let requestIndex =
					await DepositManager_CONTRACT?.withdrawalRequestIndex(
						layer2,
						account,
					);

				for (const _ of range(numPendingRequests)) {
					const req = await DepositManager_CONTRACT?.withdrawalRequest(
						layer2,
						account,
						requestIndex,
					);
				...
				const withdrawableAmount = withdrawableList.reduce(
					reduceAmount,
					initial,
				);
				const notWithdrawableAmount = notWithdrawableList.reduce(
					reduceAmount,
					initial,
				);
				...
				if (oldAddress) {
					...
					if (Old_DepositManager_CONTRACT) {
						const old_numPendingRequests =
							await Old_DepositManager_CONTRACT.numPendingRequests(
								oldAddress,
								account,
							);
						let old_requestIndex =
							await Old_DepositManager_CONTRACT.withdrawalRequestIndex(
								oldAddress,
								account,
							);
						...
						for (const _ of range(old_numPendingRequests)) {
							const old_req =
								await Old_DepositManager_CONTRACT.withdrawalRequest(
									oldAddress,
									account,
									old_requestIndex,
								);
					...
					const old_withdrawableAmount = old_withdrawableList.reduce(
						reduceAmount,
						initial,
					);
					const old_notWithdrawableAmount = old_notWithdrawableList.reduce(
						reduceAmount,
						initial,
					);
	...
	const withdrawRequests = useCallback(
		async (layer2: string) => {
			if (account && DepositManager_CONTRACT && layer2) {
				...
				const numPendingRequests =
					await DepositManager_CONTRACT.numPendingRequests(layer2, account);
				let requestIndex = await DepositManager_CONTRACT.withdrawalRequestIndex(
					layer2,
					account,
				);
				let withdrawDelay =
					await DepositManager_CONTRACT.withdrawalDelay(layer2);
				let globalWithdrawalDelay =
					await DepositManager_CONTRACT.globalWithdrawalDelay();

				withdrawDelay =
					Number(withdrawDelay.toString()) >
					Number(globalWithdrawalDelay.toString())
						? Number(withdrawDelay.toString())
						: Number(globalWithdrawalDelay.toString());
				...
				for (const _ of range(numPendingRequests)) {
					const request = await DepositManager_CONTRACT.withdrawalRequest(
						layer2,
						account,
						requestIndex,
					);
					...
					const withdrawable =
						withdrawableBlock < currentBlock.number ? true : false;
					const remainTimes = (withdrawableBlock - currentBlock.number) * 12;
					const withdrawableTime = withdrawable
						? "Withdrawable"
						: remainTimes.toString();

					const data = {
						amount: request.amount,
						time:
							withdrawableTime === "Withdrawable"
								? "Withdrawable"
								: calcCountDown2(withdrawableTime),
		...
		async (layer2: string) => {
			if (withdrawalAndDepositeds && account) {
				const filter = withdrawalAndDepositeds.filter((data: any) => {
					return (
						data.candidate.id === layer2 &&
						data.sender === account.toLowerCase()
					);
				});
				return filter;
			}
