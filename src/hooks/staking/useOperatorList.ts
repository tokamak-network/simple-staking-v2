    ...
    async function fetchList() {
      ...
      const operators = await Promise.all(
        data.map(async (obj: any) => {
          ...
          const pendingWithdrawal = pendingUnstakedLayer2
            ? convertNumber({
          ...
          totalStake = obj.updateCoinageTotalString
            ? totalStake.add(obj.updateCoinageTotalString)
            : BigNumber.from('0');
          ...
          return find ? await { ...fetchedData, name: find.name } : await fetchedData;
        }),
      );
