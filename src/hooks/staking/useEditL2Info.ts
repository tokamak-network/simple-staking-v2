import { editL2Info_logo_input, editL2Info_logo_state, editL2Info_bridge_input, editL2Info_bridge_state } from "@/atom/staking/editL2Info";
import { editL2Info_explorer_input, editL2Info_explorer_state } from "@/atom/staking/editL2Info";
import { useEffect } from "react";
import { Resetter, SetterOrUpdater, useRecoilState, useRecoilValue, useResetRecoilState } from "recoil";


function useeditL2InfoInput<T>(
  key: string,
): {
  inputValue: T | any;
  value: any | any;
  ...
  const [bridgeValue, setBridgeValue] = useRecoilState(
    editL2Info_bridge_input
  );
  const reseteditL2InfoBridgeValue = useResetRecoilState(editL2Info_bridge_input);

  const editL2Info_explorer_inputValues = useRecoilValue(editL2Info_explorer_state);
  const [explorerValue, setExplorerValue] = useRecoilState(
    editL2Info_explorer_input
  );
  const reseteditL2InfoExplorerValue = useResetRecoilState(editL2Info_explorer_input);


  const editL2Info_logo_inputValues = useRecoilValue(editL2Info_logo_state);
  const [logoValue, setLogoValue] = useRecoilState(
