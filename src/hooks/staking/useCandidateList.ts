...
export function useCandidateList() {
	...
	useEffect(() => {
		...
		async function fetchCandidateDetails(candidateObj: any, index: number) {
			const { candidateContract, stakedUserList, candidate, name } =
				candidateObj;
			let stakeOf,
				sumPending,
				stakeOfCandidate,
			...
			if (
				...
				try {
					if (account) {
						stakeOf = await SeigManager_CONTRACT.stakeOf(
							candidateContract,
							account,
						);
						myPending = await DepositManager_CONTRACT.pendingUnstaked(
							candidateContract,
							account,
						);
						stakeOfCandidate = await SeigManager_CONTRACT.stakeOf(
							candidateContract,
							candidate,
						);
						...
						if (mobile && stakeOf !== "0" && TON_CONTRACT) {
							...
							const TotContract = getContract(
								totAddress,
								Coinage,
								library,
								account,
							);
							const coinageAddress =
								await SeigManager_CONTRACT.coinages(candidateContract);
							const coinageContract = getContract(
								coinageAddress,
								Coinage,
								library,
								account,
							...
							const fromBlockNumber =
								await SeigManager_CONTRACT.lastCommitBlock(candidateContract);
					...
					const pending =
						await DepositManager_CONTRACT.pendingUnstakedLayer2(
							candidateContract,
						);
					if (oldCandidate) {
						const oldPending =
							await Old_DepositManager_CONTRACT.pendingUnstakedLayer2(
								oldCandidate,
							);
			...
			return {
				...
				stakeOfCandidate: stakeOfCandidate
					? stakeOfCandidate.toString()
					: "0.00",
				oldHistory,
				oldCommitHistory,
				myPending,
