	...
	useEffect(() => {
		async function fetchList() {
			if (account && users) {
				...
				await Promise.all(
					pastData.map(async (obj: any) => {
						const history = await getOperatorUserHistory(
							obj.layer2.toLowerCase(),
							account.toLowerCase(),
						);
						const pendingRequests = await withdrawRequests(
							obj.layer2.toLowerCase(),
						);
						const unstakeHistoryPerLayer = users[0].unstaked.filter(
							(unstake: any) =>
								unstake.candidate.id === obj.layer2.toLowerCase(),
						);
							...
							for (let i = 0; unstakeHistoryPerLayer.length > i; i++) {
								const withdrawable = pendingRequests.find((request: any) => {
									return (
										...
										Number(unstakeHistoryPerLayer[i].transaction.blockNumber) +
