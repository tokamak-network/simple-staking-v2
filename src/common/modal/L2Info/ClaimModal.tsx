...
function ClaimModal() {
	...
	const { operatorManager, claimable } = useIsOperator(
		selectedModalData?.contractAddress,
	);

	const updateSeig = useCallback(
		async (type: number) => {
			if (account && library && selectedModalData) {
				try {
					...
					const OperatorManager_CONTRACT = getContract(
						operatorManager,
						OperatorManager,
						library,
						account,
					);
					const tx = await OperatorManager_CONTRACT.claimERC20(
						WTON_ADDRESS,
						claimable,
					);
						...
						<ModalBody>
							...
							>
								...
								<Flex
									...
									fontSize={"13px"}
								>
									<Flex fontWeight={500} color={"#808992"} fontSize={"12px"}>
                    ...
                    {selectedModalData.target ? 
                      trimAddress({
                        ...
                        dots:' ...'
									...
									<Flex fontWeight={500} color={"#3e495c"} fontSize={"13px"}>
									...
									<Flex fontSize={"13px"} color={"#808992"} fontWeight={500}>
										{selectedModalData.name === "Claim" ? "to" : "using the"}
										<span
											style={{
												marginLeft: "3px",
												marginRight: "3px",
								...
								<Flex>
									<Button
										...
										onClick={() =>
											updateSeig(selectedModalData.name === "Claim" ? 1 : 2)
										}
									>
