...
export const WithdrawToEthereum = (args: WithdrawToEthereumProps) => {
	...
	const { pendingUnstaked, loading } = usePendingUnstaked(
	...
	useEffect(() => {
		const fetch = async () => {
			if (selectedModalData) {
				//@ts-ignore
				const pendingRequests = await withdrawRequests(
					selectedModalData.layer2,
				);
		...
		<Flex flexDir={"column"} w={"350px"} alignItems={"center"}>
			...
			<Flex>
				{requests && requests.length == 0 ? (
					...
					<WithdrawTable columns={columns} data={requests} toggle={toggle} />
				) : (
					<LoadingDots />
				)}
			</Flex>
			<Flex
				...
				alignItems={"center"}
			>
				<Flex fontSize={"12px"} fontWeight={500} color={"#808992"} mb={"5px"}>
					Total amount to{" "}
					{toggle === "Withdraw" ? (
				...
				<Flex fontSize={"18px"} fontWeight={500} color={"#3d495d"}>
					<Flex mr={"9px"}>
						{
							// loading  ? <LoadingDots /> :
							withdrawLoading ? (
								...
								<BalanceTooltip
									label={
										requests.length === 0 ? "0" : pendingUnstaked.toString()
									}
									types={"ray"}
								/>
							)
			...
			<Flex flexDir={"column"} alignItems={"center"}>
				...
				<Button
					...
					_hover={toggle === "Restake" ? { bgColor: "#36af47" } : ""}
