...
import {
	useWithdrawable,
	useWithdrawRequests,
} from "@/hooks/staking/useWithdrawable";
...
function WithdrawModal() {
	...
	useEffect(() => {
		if (selectedModalData) {
			const infos = L2Info.find(
				(info: any) => info.candidate === selectedModalData.candidate,
			);
	...
	useEffect(() => {
		...
		if (selectedModalData) {
			...
			numberOf =
				selectedModalData.old_withdrawable !== "0.00" ? numberOf + 1 : numberOf;
		}
		setNumber(numberOf);
	}, [selectedModalData]);
					...
					{selectedModalData ? (
						<ModalBody>
							...
							>
								<Flex
									flexDir={"row"}
									justifyContent={type !== "main" ? "start" : "center"}
									w={"100%"}
								>
									{type !== "main" ? (
										<Flex
									...
									>
										{selectedModalData.old_withdrawable !== "0.00" ? (
											<WithdrawType
												name={"Prior to Patch"}
												content={
													"Any unstaked TON from the original staking contract with the zero-day vulnerability can be withdrawn to Ethereum."
												}
											/>
										) : (
											""
										...
										<WithdrawType
											name={"Ethereum"}
											content={
												"Staked TON can be unstaked and can be withdrawn after 93,046 blocks from unstaking (~14 days)."
											}
											src={ETHEREUM.src}
										...
										{selectedModalData.isL2 ? (
											<WithdrawType
												name={"L2"}
												content={
													"Instead of withdrawing to Ethereum, staked TON can be withdrawn to this layer as TON. By withdrawing to this layer, TON can be used right away without needing to wait for 14 days."
												}
												src={logo ? logo : "L2"}
