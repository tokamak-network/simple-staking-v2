...
export const ToTitan = (args: ToTitanProps) => {
	...
	const { rollupConfigInfo } = useIsOperator(selectedModalData?.layer2);
	...
	const { stakedAmount } = selectedModalData;
	...
	return (
		<Flex flexDir={"column"}>
			...
			/>
			<UnstakeBalanceInput stakedAmount={stakedAmount} />
			...
			{Number(rollupConfigInfo) === 2 ? (
				<Text
					...
					ml={"10px"}
				>
					<span style={{ color: "#ff2d78" }}>Warning</span>: You cannot withdraw
					to this layer.
				</Text>
			) : (
