...
export const WithdrawL2TableRow: FC<WithdrawL2TableRowProps> = ({
	...
	const { amount, timestamp, l2txHash, id } = cell.row?.original;
	const [duration, setDuration] = useState("0");

	const values = amount;

	const type = cell.column.id;
	const usdValue = ((tonPrice * +values) / Math.pow(10, 27)).toLocaleString(
		undefined,
	...
	useEffect(() => {
		...
		setDuration(
			`${year}.${month}.${day}.${hour}:${minute} (GMT+${difference})`,
		);
	}, []);
