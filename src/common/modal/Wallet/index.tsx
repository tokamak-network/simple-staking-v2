...
function WalletModal() {
	const { account, connector, activate, error, active, deactivate, chainId } =
		useWeb3React();
					...
					if (option.name === "MetaMask") {
						return (
							<WalletOption
								...
								icon={
									require("../../../assets/images/" + option.iconName).default
								}
								size={"20px"}
							/>
						);
		...
		<Modal isOpen={selectedModal === "wallet"} onClose={closeModal}>
			{walletView === WALLET_VIEWS.ACCOUNT && account ? (
				...
				>
					<ModalHeader>
						{Number(DEFAULT_NETWORK) !== chainId ? (
							<Flex h={"200px"} justifyContent={"end"} flexDir={"column"}>
								<Flex mb={"15px"}>
