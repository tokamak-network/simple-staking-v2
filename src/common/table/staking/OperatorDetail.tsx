...
export const OperatorDetailInfo: FC<OperatorDetailProps> = ({
	...
	return (
		<Flex flexDir={"column"} minW={"175px"} mr={"30px"}>
			<Text color={"#808992"} fontSize={"12px"} mb={"9px"}>
				{title}
			</Text>
			<Flex flexDir={"column"} alignItems={"start"} h={"37px"}>
				<Flex flexDir={"row"}>
					...
					>
						{title === "Unclaimed Staking Reward" && !minimumAmount ? (
							...
							<BalanceTooltip label={value} types={"ray"} />
						)}
					</Text>
					{unit ? (
						<Text fontSize={"13px"} fontWeight={500} ml={"6px"} mt={"5px"}>
							{unit}
						</Text>
					) : (
						""
				...
				<Flex h={"13px"} fontSize={"11px"} fontWeight={400}>
					...
					<Flex>
						<BalanceTooltip label={totalValue} types={"ray"} />
						<Flex ml={"3px"}>TON</Flex>
					</Flex>
				</Flex>
			</Flex>
			{title === "Unclaimed Staking Reward" && !minimum && account ? (
				<Text
					...
					// w={'250px'}
				>
					<span style={{ color: "#ff2d78" }}>Warning</span>: The
					<Link
						isExternal
					...
					on this layer2 before stakers can receive the staking reward. Users
					are unable to stake until this minimum collateral requirement is
					fulfilled.
				</Text>
				...
				>
					<Flex alignItems={"center"}>
						<Flex mr={"3px"}>Update seigniorage</Flex>
						<BasicTooltip
							label={
								"Update Seigniorage: Request the seigniorage distribution to claim your accrued staking rewards based on the current L2 deposits and staked TON."
							}
						/>
					</Flex>
				</Flex>
			) : title === "Staked" && account ? (
				<Flex
					fontSize={"11px"}
					color={"#2a72e5"}
					cursor={"pointer"}
				...
				>
					<Flex alignItems={"center"}>
						<Flex mr={"3px"}>Withdraw</Flex>
						<BasicTooltip
							label={
								"Withdraw Staked TON: Initiate a withdrawal of your staked TON. Note that the withdrawal process may be subject to L2 network conditions and dispute periods."
							}
						/>
					</Flex>
				</Flex>
			) : title === "Pending Withdrawal" && account ? (
				<Flex
					fontSize={"11px"}
					color={"#2a72e5"}
					cursor={"pointer"}
				...
				>
					<Flex alignItems={"center"}>
						<Flex mr={"3px"}>Restake</Flex>
						<BasicTooltip
							label={
								"Restake Pending Withdrawal: Reinvest your pending withdrawal TON back into staking to continue earning rewards and help secure the network."
							}
						/>
					</Flex>
				</Flex>
