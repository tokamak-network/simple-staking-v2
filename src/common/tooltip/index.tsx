...
const BasicTooltip: React.FC<tooltipProps> = (props) => {
	...
	const { label, label2, placement, types, size, width } = props;
	...
	const candidate = () => {
        ...
        > */}
				registered
				{/* </Link> */}
				as a DAO candidate who has staked a minimum of 1,000.1 TON
				(non-withdrawable).
			</Text>
		);
	};
				...
				{seigRatioInfo.map((props: any, index: number) => {
					return (
						...
						>
							...
							<Flex>{props.ratio}</Flex>
							<Flex fontSize={"10px"} fontWeight={400} color={"#ccc"}>
								{props.formular}
							</Flex>
						</Flex>
					);
