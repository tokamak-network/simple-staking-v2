...
import { chakra, Text, Flex, Box, useTheme } from "@chakra-ui/react";
...
export const OpearatorInfos: FC<OpearatorInfoProps> = ({
	...
	const { operatorManager, managers, bridge, rollupConfig } =
		useIsOperator(candidateContract);
	...
	const candidateAmount = stakeOfCandidate
		? convertNumber({
	...
	const isMember = memberAddresses?.find(
		(address: any) => address.member === candidate,
	);

	const yourStaked = stakeOf
		? convertNumber({
				//@ts-ignore
				amount: stakeOf,
				type: "ray",
	...
	return (
		<Flex flexDir={"column"}>
			<Flex
				...
				cursor={"pointer"}
				borderRadius={"10px"}
				borderBottomRadius={isOpen === candidateContract ? "0px" : "10px"}
				borderBottom={isOpen === candidateContract ? "solid 2px #f4f6f8" : ""}
				borderBottomColor={isOpen === candidateContract ? "#f4f6f8" : ""}
				px={"16px"}
				mb={"15px"}
				w="100%"
				bg={"white.100"}
			...
			>
				<Flex alignItems="center">
					...
					<Info
						...
						unit={"%"}
					/>
					<Info title={"Total Staked"} value={stakedAmount} unit={"TON"} />
					{yourStaked !== "0.00" && yourStaked ? (
						<Info title={"Your Staked"} value={stakeOf} unit={"TON"} />
					) : (
						""
					)}
				</Flex>
				<RenderBtn layer2={candidateContract} isOpen={isOpen} />
			</Flex>
				...
				>
					<Flex flexDir={"column"}>
						...
						>
							<Flex>
								{candidateAddOn !== null ? (
									<InfoTypeSelector tab={tab} setTab={setTab} />
								) : (
									""
								)}
							</Flex>
							<Flex
								...
								color={"#304156"}
							>
								<Flex mr={"6px"}>L1 Contract address</Flex>
								<ContractAddressInfo
