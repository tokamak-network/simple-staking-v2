...
function OperatorCard(props: { operator: any }) {
	...
	const { withdrawable, notWithdrawable } = useWithdrawable(
	...
	const candidateAmount = candidate?.stakeOfCandidate
		? convertNumber({
	...
	const earned = operator?.candidateAddOn?.seigGiven[0]
		? convertNumber({
	...
	const l2Tabs = [
		...
		{
			title: "Sequencer seigniorage",
			value: "title",
		},
		{
			title: "TON locked in Bridge",
			value: lockedInBridges,
		},
		// {
				...
				<Flex w="100%" h="100%" flexDir={"column"} px="20px">
					...
					{account && !minimumAmount ? (
						...
						>
							...
							on this layer2 before stakers can receive the staking reward.
							Users are unable to stake until this minimum collateral
