...
function L2Information({ data }: L2InformationProps) {
	...
	const [claimableAmount, setClaimableAmount] = useState<string | undefined>(
		"0.00",
	);
	...
	const [chainIdValue, setChainIdValue] = useRecoilState(
		editL2Info_chainId_input,
	);
	...
	const modalButton = useCallback(
		async (modalType: ModalType, name: string, data: any) => {
			// setName(name)
		...
		>
			<Flex flexDir={"column"} mt={"36px"}>
				...
				<Flex flexDir={"row"}>
					...
					<L2InfoContent
						...
						content={explorerValue}
						type={"explorer"}
						editStat={editStat}
					/>
					<L2InfoContent
						title={"Chain Id"}
						content={chainIdValue}
						type={"string"}
						editStat={editStat}
					/>
					<Flex
			...
			<Flex flexDir={"column"} my={"54px"}>
				<Flex fontSize={"18px"} fontWeight={500} color={"#2a72e5"} mb={"18px"}>
					<Flex>Sequencer seigniorage</Flex>
					<Flex mt={"9px"} ml={"5px"}>
						<BasicTooltip
							label={
								"Sequencer seigniorage represents the portion of newly issued TON that the L2 operator can claim as a reward. This reward increases with the growth of L2 deposits and helps incentivize both the expansion and security of the network."
							}
						/>
					</Flex>
					{Number(rollupConfigInfo) === 2 ? (
						<Flex
							...
							ml={"10px"}
						>
							<span style={{ color: "#ff2d78" }}>Warning</span>: This layer has
							stopped the l2 sequencer seigniorage.
						</Flex>
					) : (
						""
					)}
					...
					{isOperator ? (
						...
						>
							<Button
								...
								onClick={() => {
									modalButton("claim", "Claim", dataModal);
								}}
							>
								Claim
							</Button>
