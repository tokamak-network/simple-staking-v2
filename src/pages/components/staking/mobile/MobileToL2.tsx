...
export function MobileToL2(args: MobileToL2Props) {
	...
	const myStaked = selectedOp?.stakeOf
		? convertNumber({
	...
	return (
		<Flex flexDir={"column"}>
			<WithdrawL2Image l2Image={logo ? logo : ""} l2Name={selectedOp?.name} />
			<UnstakeBalanceInput stakedAmount={myStaked ? myStaked : "0.00"} />
			<StakingCheckbox
			...
			{withdrawTx && withdrawTx.length > 0 ? (
				<WithdrawL2Table columns={columns} data={withdrawTx} />
			) : (
				""
			)}
		</Flex>
