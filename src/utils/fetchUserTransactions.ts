...
export const fetchUserTransactions = async (
	...
	if (account) {
		const { L1Bridge, DepositManager_ADDRESS, TON_ADDRESS } = CONTRACT_ADDRESS;
		...
		const resTxs = await axios.post(`${GRAPHQL_API}`, {
			query: `
        {
          sentMessages(
		...
		const withdrawTx = await axios.post(`${TITAN_SUBGRAPH}`, {
			query: `
        {
          sentMessages(
            where: {
		...
		const allDepL1Txs = [...resTxs.data.data.erc20DepositInitiateds];
